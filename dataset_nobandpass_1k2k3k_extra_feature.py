...
def compute_response_for_tone(recorded, sr, f_tone):
    ...
    # store FFT result of each window
    magnitudes = []
...
def get_baseline(subject_dir):
    """
    get baseline value from subject_dir
    :param subject_dir: subject directory (e.g., 'A1' or 'A2')
    :return: a dictionary with 3 frequency baseline values, if failed, the corresponding frequency value is 0
    """
    # print(subject_dir)
    # 初始化 baseline 字典
    baseline_dict = {
    ...
    if not os.path.isfile(source_audio_path):
        logging.warning(f"Can't find baseline file: {source_audio_path}")
...
def process_file(file_path, output_dir, case_number, last_num, num_samples=6):
    ...
    if case_number == 1:
        ...
        for i, freq in enumerate(freqs, start=1):
            ...
            # If you want to add the bandpass filter, you can use the following code:
            # sf.write(os.path.join(output_dir, wav_filename), filtered_segment, sample_rate)
    ...
    for filename in sorted(os.listdir(subject_dir)):
        if filename.endswith('.wav'):
            ...
            if case_number not in [1, 2, 3, 4]:
                logging.info(f"Skipping non-case1-4 file: {filename}")
                continue
...
def process_all_participants(root_dir, output_root_dir, num_samples=6):
    ...
    for participant in os.listdir(root_dir):
        ...
        if os.path.isdir(participant_dir):
            ...
            logging.info(f"Participant {participant} sample count: {participant_sample_count}")
...
if __name__ == "__main__":
    ...
    logging.info(f"Processing completed. Total sample count: {total_samples}")    