...
def process_file(
    ...
    if case_number == 1:
        ...
        for i, freq in enumerate(freqs, start=1):
            ...
            sf.write(os.path.join(output_dir, wav_filename), segment_no_filter, sample_rate)
...
def process_subject_directory(subject_dir, output_dir, num_samples=6):
    ...
    if not os.path.isfile(source_audio_path):
        logging.warning(f"目录 {subject_dir} 下找不到 case1_1.wav")
...
if __name__ == "__main__":
    ...
    output_root_directory = './dataset_2k3k_withbandpass_extrafeatures_v3'
