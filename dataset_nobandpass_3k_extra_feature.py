...
# add global variables before function definitions
cases = {
    1: [3000, 2000, 1000],
...
def compute_response_for_tone(recorded, sr, f_tone):
    ...
    # store FFT results for each window
    magnitudes = []
...
def get_baseline(subject_dir):
    """
    Get baseline values from the specified directory
...
def process_file(file_path, output_dir, case_number, last_num, num_samples=6):
    ...
    if case_number == 1:
        ...
        for i, freq in enumerate(freqs, start=1):
            if freq != 3000:  # only process 3k frequency
                continue
...
def process_subject_directory(subject_dir, output_dir, num_samples=6):
    ...
    for filename in sorted(os.listdir(subject_dir)):
        if filename.endswith('.wav'):
            ...
            # check if it is a 3k frequency file
            if case_number != 1:
                freq = cases[case_number][last_num]
                if freq != 3000:
                    continue
            ...
            if case_number == 1:
                sample_count += 6  # case1 now only has 3k samples, so it is 6
            else:
                # only count if it is a 3k frequency file
                if cases[case_number][last_num] == 3000:
...
if __name__ == "__main__":
    ...
    output_root_directory = './dataset_3k_withbandpass_extrafeatures'
